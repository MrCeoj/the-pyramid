...
import { match, team, pyramid, profile, position, positionHistory } from "@/db/schema";
...
export async function getAcceptedMatches(): Promise<AcceptedMatchWithDetails[]> {
  try {
    ...
    await db.transaction(async (tx) => {
      ...
      if (attackerWins) {
        ...
        await tx
          .update(team)
          .set({
            wins: challengerTeamId === winnerTeamId ? 
              (await tx.select({ wins: team.wins }).from(team).where(eq(team.id, challengerTeamId)).limit(1))[0].wins + 1 : 
              (await tx.select({ wins: team.wins }).from(team).where(eq(team.id, challengerTeamId)).limit(1))[0].wins
          })
        ...
        await tx
          .update(team)
          .set({
            losses: (await tx.select({ losses: team.losses }).from(team).where(eq(team.id, defenderTeamId)).limit(1))[0].losses + 1
          })
        ...
        await tx
          .update(team)
          .set({
            wins: (await tx.select({ wins: team.wins }).from(team).where(eq(team.id, defenderTeamId)).limit(1))[0].wins + 1
          })
          .where(eq(team.id, defenderTeamId));
        ...
        await tx
          .update(team)
          .set({
            losses: (await tx.select({ losses: team.losses }).from(team).where(eq(team.id, challengerTeamId)).limit(1))[0].losses + 1
          })
          .where(eq(team.id, challengerTeamId));
